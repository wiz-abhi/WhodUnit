...
**Zero false culprits across six** — the claim that matters, since it's the one the tool can't fake by construction. Whodunit nails the flagship conjunctive fault the flat baseline cannot (`conditional_dep`: `(A => B) && NOT C`, recall 1.0, baseline top-feature precision 0.23); ties the baseline on the single-feature fault (`new_edge`); and takes the honesty path — calibrated ABSTAIN/PARTIAL, never a false culprit — on the inexpressible (`retry_storm`), the decoy trap (`decoys`), and the null cohort (`null_scenario`). It also recovers the pure trace-scoped absence (`cache_bypass`: `NOT cache-get`) that the flat baseline finds — the pipeline now anchors the absence to an always-present positive (`... && NOT cache-get`) so it compiles and verifies (recall 1.0, ISSUES.md #2 FIXED). By the scorecard that's **6/6**; read row-by-row it's one differentiated conjunction, one honest tie, one recovered absence, one below-confidence partial, and two correct abstentions — with, crucially, no confident wrong answer anywhere.

> Caveat worth stating plainly: this is the author's fault-injection corpus scored by the author's harness, so it demonstrates **compile-and-verify correctness on disclosed, machine-labelled faults**, not discovery on real production traffic. The unfakeable figure is the differential receipt (mined vs live SigNoz) and `recall == 1.0`; the "pass" counts are honest but self-graded. See `../docs/STATS-NOTES.md` for the effect-measure and FDR refinements still on the list.
